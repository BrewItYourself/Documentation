...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.30]{servo-large-actuator-circuit.png}
\caption{Large Servo Motor Power Circuit}
\label{fig:servo-large-actuator-circuit}
\end{center}
\end{figure}

However, for smaller servos that require less current to function, the relay and external DC source can be removed from the circuit completely, and the motor can be powered directly from the micro-controller's 5V output pins, as depicted in Figure \ref{fig:servo-small-actuator-circuit}.
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.30]{heater-hysteresis-circuit.png}
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.30]{motor-thyristor-dc-circuit.png}
